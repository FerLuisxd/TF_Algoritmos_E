...
public:
	Universo();
	~Universo();
	Universo(function<void(T, T)> compare) : compare(compare) {}
	unsigned int length();
	int height();
	void add(T element);
...
template <typename T>
Universo<T>::Universo() : root(nullptr) {
...
template <typename T>
unsigned int Universo<T>::length() {
	return length(root);
}

template <typename T>
unsigned int Universo<T>::length(Node* node) {
	return node != nullptr ? node->length : 0;
}

template <typename T>
int Universo<T>::height() {
	return height(root);
}

template <typename T>
int Universo<T>::height(Node* node) {
	return node != nullptr ? node->height : -1;
}

template <typename T>
void Universo<T>::update(Node* node) {
	if (node != nullptr) {
...
template <typename T>
void Universo<T>::rotateLeft(Node*& node) {
...
template <typename T>
void Universo<T>::rotateRight(Node*& node) {
...
template <typename T>
void Universo<T>::balance(Node*& node) {
...
template <typename T>
void Universo<T>::add(T element) {
	add(root, element);
}

template <typename T>
void Universo<T>::add(Node*& node, T element) {
	if (node == nullptr) {
...
template <typename T>
bool Universo<T>::remove(T element) { return false; }
template <typename T>
bool Universo<T>::remove(Node*& node, T element) { return false; }

template <typename T>
T Universo<T>::search(T key) {
	return search(root, key);
}

template <typename T>
T Universo<T>::search(Node* node, T key) {
	if (node != nullptr) {
...
template <typename T>
void Universo<T>::preorder(std::function<void(T)> proc, int limit) {
	int i = 0;
	preorder(root, proc, i, limit);
}

template <typename T>
void Universo<T>::preorder(Node* node, std::function<void(T)> proc, int& i, int limit) {
...
template <typename T>
void Universo<T>::inorder(std::function<void(T)> proc, int limit) {
	int i = 0;
	inorder(root, proc, i, limit);
}

template <typename T>
void Universo<T>::inorder(Node* node, std::function<void(T)> proc, int& i, int limit) {
	if (node != nullptr && i < limit) {
...
template <typename T>
void Universo<T>::postorder(std::function<void(T)> proc, int limit) {
	int i = 0;
	postorder(root, proc, i, limit);
}

template <typename T>
void Universo<T>::postorder(Node* node, std::function<void(T)> proc, int& i, int limit) {
	if (node != nullptr && i < limit) {
...
template <typename T>
void Universo<T>::bfs(std::function<void(T)> proc, int limit) {
	int i = 0;
	std::queue<Node*> qu;
	qu.push(root);
